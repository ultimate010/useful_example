...
class MediaInfo:
    ...
    try:
     if raw_value:
       if "kbps" in raw_value:
         ...
         return int(float(re.search('\d+',value).group()))
       elif "mbps" in raw_value:
         value = self._get_stripped_value("mbps", label)
         value = int(float(re.search('\d+',value).group())) * 1024
         return value
    except:
      return None
  ...
  def get_frames_per_second(self):
    ...
    if not value:
      value = self._get_stripped_value("fps", "video_nominal_frame_rate")
    try:
...
class VideoConverter:
  ...
  def _get_convert_commands(self, original, output):
    ...
    # width = 1000 # mediainfo.get_width()
    # height = 100 # mediainfo.get_height()
    # fps = 30 # mediainfo.get_frames_per_second()
    # video_bitrate = 1000 # mediainfo.get_video_bitrate()
    # audio_bitrate = 100 # mediainfo.get_audio_bitrate()
    # audio_sampling = 100 # mediainfo.get_audio_sampling()
    ...
    for filetype in self.formats:
      ...
      commands.append((filetype, self._get_convert_command(filetype, original,
         os.path.join(self.parent_dir, tmpfile), 0, 0,
         0, 0, 0)))
